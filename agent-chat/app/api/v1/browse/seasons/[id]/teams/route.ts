/**
 * Browse Season Teams API Route Handler
 *
 * Proxies browse teams requests to the backend FastAPI server.
 *
 * @module app/api/v1/browse/seasons/[id]/teams/route
 */
 ...
 *
 * Returns teams for a specific season.
 */
export async function GET(_request: Request, { params }: RouteParams) {
  try {
    const { id } = await params;
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/v1/browse/seasons/${id}/teams`
    );
