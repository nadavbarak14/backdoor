"""
Winner League Exceptions Module

Provides custom exception classes for Winner League data fetching operations.
These exceptions allow for specific error handling and meaningful error messages.

This module exports:
    - WinnerError: Base exception for all Winner League errors
    - WinnerAPIError: HTTP/API request failures
    - WinnerParseError: Data parsing failures
    - WinnerRateLimitError: Rate limit exceeded
    - WinnerTimeoutError: Request timeout

Usage:
    from src.sync.winner.exceptions import WinnerAPIError, WinnerParseError

    try:
        data = client.fetch_boxscore("12345")
    except WinnerAPIError as e:
        print(f"API error: {e}")
    except WinnerParseError as e:
...
class WinnerError(Exception):
    """
    Base exception for Winner League operations.

    All Winner League specific exceptions inherit from this class,
    ...
    Example:
        >>> try:
        ...     raise WinnerError("Something went wrong")
        ... except WinnerError as e:
        ...     print(f"Winner error: {e}")
        Winner error: Something went wrong
    """

    def __init__(self, message: str) -> None:
        """
        Initialize WinnerError.
...
class WinnerAPIError(WinnerError):
    """
    Exception raised for HTTP/API request failures.

    Raised when an HTTP request to Winner League APIs fails,
...
class WinnerParseError(WinnerError):
    """
    ...
    Example:
        ...
        Parse error for boxscore: Missing 'teams' field
    """
...
class WinnerRateLimitError(WinnerError):
    """
    ...
    def __init__(
        ...
        """
        Initialize WinnerRateLimitError.
...
class WinnerTimeoutError(WinnerError):
    """
    ...
    Example:
        >>> try:
        ...     raise WinnerTimeoutError(
        ...         "Request timed out",
        ...         timeout=30.0,
        ...         url="https://example.com/api"
        ...     )
        ... except WinnerTimeoutError as e:
        ...     print(f"Timeout after {e.timeout}s: {e.url}")
        Timeout after 30.0s: https://example.com/api
    """
